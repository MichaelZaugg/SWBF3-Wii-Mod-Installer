...
ICON_PATH = os.path.join(CURRENT_DIR, 'SWBF3Icon.ico')
...
def print_to_console(message, tag=None):
    """Print messages to the console UI and flush immediately."""
...
def show_about():
    ...
    Please download the unpacked version for modding

    Run Dolphin at least once before installing these mods

    Please make sure to download and extract Mods.zip.
    """)
...
def browse_folder(entry_widget, update_global):
    ...
    if folder_selected:
        if update_global == "game":
            ...
            result_path = find_main_dol_dir(folder_selected)
            if result_path:
                global GLOBAL_MOD_DIR
                GLOBAL_MOD_DIR = result_path
                print_to_console(f"Mod directory set to: {GLOBAL_MOD_DIR}", 'directory')
                entry_widget.delete(0, tk.END)
                entry_widget.insert(0, GLOBAL_MOD_DIR)
            else:
                print_to_console("No 'main.dol' file found in the selected directory.", 'error')
                entry_widget.delete(0, tk.END)
                entry_widget.insert(0, "Invalid mod directory. 'main.dol' not found.")
                GLOBAL_MOD_DIR = ""
...
def find_main_dol_dir(base_path):
    for root, dirs, files in os.walk(base_path):
        if 'main.dol' in files:
            return root
    return None
...
def search_dolphin_emulator(entry_widget):
    ...
    if found_paths:
        entry_widget.delete(0, tk.END)
        entry_widget.insert(0, found_paths[0])
        GLOBAL_APPDATA_DIR = found_paths[0]
        GLOBAL_CUSTOM_APPDATA = False
        print_to_console(f"AppData Dir: {found_paths[0]}", 'directory')
    else:
        entry_widget.delete(0, tk.END)
...
def setup_top_buttons(root):
    ...
    # Load the image to display as a fun image
    try:
        fun_image = PhotoImage(file=os.path.join(CURRENT_DIR, "image.png"))  # Replace with your image path
...
def setup_directory_ui(root):
    ...
    Button(root, text="Browse Mod Directory", command=lambda: browse_folder(mod_dir_entry, "mod")).grid(row=2, column=2, padx=5)
...
def reset_appdata_path(entry_widget):
    global GLOBAL_CUSTOM_APPDATA
    search_dolphin_emulator(entry_widget)
    GLOBAL_CUSTOM_APPDATA = False
    print_to_console("AppData path reset to default Dolphin Emulator directory.", 'directory')
