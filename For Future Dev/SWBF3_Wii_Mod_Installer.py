    ...
    def perform_update_sequence():
        try:
            ...
            if not GLOBAL_MOD_DIR:
                log_message("Please select the mod directory.", "error")
                return
            ...
            existing_mods = {folder.name for folder in Path(GLOBAL_MOD_DIR).iterdir() if folder.is_dir()}

            for mod in manifest.get("mods", []):
                expected_folder_name = mod["file"].replace(".zip", "")
                if expected_folder_name not in existing_mods:
                    updates_required.append(mod)
                else:
                    log_message(f"{expected_folder_name} is up to date.", "success")

            if updates_required:
                log_message("Updates required for the following mods:", "info")
                print_to_console("This may take a while depending on your internet connection", 'directory')
                for mod in updates_required:
                    log_message(f"- {mod['name']}", "info")
                download_updates(updates_required)
            else:
                log_message("All mods are up to date!", "success")
        finally:
            stop_loading()
...
def download_file_from_google_drive(file_id, destination):
    """
    Download a file from Google Drive using gdown in a separate thread.
    """
    def perform_download():
        file_url = f"https://drive.google.com/uc?id={file_id}"
        try:
            gdown.download(file_url, destination, quiet=False)
            log_message(f"Downloaded file to {destination}", "success")
        except Exception as e:
            log_message(f"Failed to download file: {e}", "error")
...
def download_updates(updates):
    """
    Download and extract mods from Google Drive, with spinner animation.
    """
    def perform_download():
        try:
            for mod in updates:
                try:
                    log_message(f"Downloading {mod['name']}...", "info")

                    # Extract the file ID from the Google Drive URL
                    file_id = mod["download_url"].split("id=")[-1].split("&")[0]
                    file_path = os.path.join(GLOBAL_MOD_DIR, mod.get("file", ""))

                    # Start download in a separate thread
                    download_thread = download_file_from_google_drive(file_id, file_path)
                    download_thread.join()  # Wait for the thread to complete

                    log_message(f"Downloaded {mod['name']} successfully.", "success")

                    # Validate if the file is a ZIP
                    if not zipfile.is_zipfile(file_path):
                        log_message(f"File {file_path} is not a valid ZIP file. Skipping extraction.", "error")
                        continue
                    ...
                    handle_duplicate_folders(extract_path)

                    log_message(f"Extracted {mod['name']} to {extract_path}.", "success")

                    # Delete the ZIP file
                    os.remove(file_path)
                    log_message(f"Deleted ZIP file for {mod['name']}.", "info")

                except Exception as e:
                    log_message(f"Unexpected error for {mod['name']}: {e}", "error")
        finally:
            root.after(0, stop_loading)  # Stop the spinner in the main thread
            log_message("All downloads are complete (or attempted)!", "success")

    root.after(0, start_loading)  # Start spinner in the main thread
    thread = threading.Thread(target=perform_download, daemon=True)
    thread.start()


def handle_duplicate_folders(extract_path):
    """
    Flatten nested folders if the extracted folder contains only one subfolder
    with the same name.
    """
    extracted_content = os.listdir(extract_path)
    if len(extracted_content) == 1:
        single_folder = os.path.join(extract_path, extracted_content[0])
        if os.path.isdir(single_folder):
            for item in os.listdir(single_folder):
                shutil.move(os.path.join(single_folder, item), extract_path)
            shutil.rmtree(single_folder)
            log_message(f"Flattened nested folder structure in {extract_path}.", "info")
...
def get_versioned_path(mod_base_name):
    """
    Helper to construct the versioned mod folder name based on the manifest.
    """
    manifest = fetch_manifest()
    if not manifest:
        log_message("Manifest not available. Ensure it is properly fetched.", "error")
        return None

    for mod in manifest.get("mods", []):
        if mod["name"].startswith(mod_base_name):
            folder_name = mod["file"].replace(".zip", "")
            return os.path.join(GLOBAL_MOD_DIR, folder_name)

    log_message(f"Mod '{mod_base_name}' not found in manifest.", "error")
    return None
...
def install_dynamic_input_textures():
    """
    Installs Dynamic Input Textures by copying the required files.
    """
    dynamic_input_textures_dir = Path(get_versioned_path("DynamicInputTextures"))
    ...
    if not dynamic_input_textures_dir.exists():
        log_message(f"Dynamic Input Textures mod directory '{dynamic_input_textures_dir}' not found. Ensure the mod is downloaded and extracted correctly.", "error")
        return
    ...
    # Copy Dynamic Input Textures files
    log_message("Copying Dynamic Input Textures files...", "info")
    copy_files(dynamic_input_textures_dir / "DynamicInputTextures", dynamic_textures_dir)

    # Copy RABAZZ textures
    log_message("Copying RABAZZ textures...", "info")
    rabazz_source = Path(get_versioned_path("RABAZZ_dynamic")) / "RABAZZ"
    if rabazz_source.exists():
        copy_files(rabazz_source, rabazz_dir)
    else:
        log_message(f"RABAZZ folder not found in '{dynamic_input_textures_dir}'. Skipping RABAZZ textures.", "warning")



def install_muted_blank_audio():
    mod_dir = get_versioned_path("SWBF3_Wii_Muted_Blank_Sounds")
    if not mod_dir:
        return  # Exit if mod directory is not found

    game_data_dir = Path(GLOBAL_GAME_DIR)
    copy_files(mod_dir, game_data_dir)

def install_4k_texture_pack():
    mod_dir = get_versioned_path("4kTexturePacks")
    if not mod_dir:
        return  # Exit if mod directory is not found
...
def install_lighting_fix():
    """
    Installs the Lighting Fix mod by copying files, compiling resources, and updating scene descriptors.
    """
    lighting_fix_dir = Path(get_versioned_path("SWBF3_Wii_Light_Fixes"))
    embedded_res_compiler_dir = Path(get_versioned_path("EmbeddedResCompiler"))
    game_data_dir = Path(GLOBAL_GAME_DIR) / "DATA" / "files"

    if not lighting_fix_dir.exists():
        log_message(f"Lighting Fix mod directory '{lighting_fix_dir}' not found. Ensure the mod is downloaded and extracted correctly.", "error")
        return

    if not embedded_res_compiler_dir.exists():
        log_message(f"Embedded Res Compiler mod directory '{embedded_res_compiler_dir}' not found. Ensure the mod is downloaded and extracted correctly.", "error")
        return

    # Copy Embedded Res Compiler files
    log_message("Copying Embedded Res Compiler files...", "info")
    copy_files(embedded_res_compiler_dir, game_data_dir)
    # Copy Lighting Fix files
    log_message("Copying Lighting Fix files...", "info")
    copy_files(lighting_fix_dir, game_data_dir)
    ...
    if compile_script_path.exists():
        ...
        log_message(f"Compile script not found at {compile_script_path}", "error")
    ...
    if scene_descriptor_file.exists():
        ...
        log_message(f"Scene descriptor file not found at {scene_descriptor_file}", "error")

    # Compile resources again for finalization
    if compile_script_path.exists():
        log_message("Compiling resources again for finalization...", "info")
        try:
            subprocess.run(
                str(compile_script_path),
                shell=True,
...
def install_updated_debug_menu():
    mod_dir = Path(get_versioned_path("main"))
    if not mod_dir.exists():
        log_message("Updated Debug Menu mod directory not found.", "error")
        return  # Exit if mod directory is not found
...
def install_cloth_fix():
    """
    Installs the Cloth Fix mod by copying files and running the resource compiler.
    """
    cloth_fix_dir = Path(get_versioned_path("Battlefront_III_Cloth_Fix"))
    embedded_res_compiler_dir = Path(get_versioned_path("EmbeddedResCompiler"))
    game_data_dir = Path(GLOBAL_GAME_DIR) / "DATA" / "files"

    if not cloth_fix_dir.exists():
        log_message(f"Cloth Fix mod directory '{cloth_fix_dir}' not found. Ensure the mod is downloaded and extracted correctly.", "error")
        return
    ...
    # Copy Cloth Fix files
    log_message("Copying Cloth Fix files...", "info")
    copy_files(cloth_fix_dir / "Battlefront_III_Cloth_Fix" , game_data_dir)

    # Copy Embedded Res Compiler files
    log_message("Copying Embedded Res Compiler files...", "info")
    copy_files(embedded_res_compiler_dir, game_data_dir)

    # Compile resources using the provided batch script
    compile_script_path = game_data_dir / "compile_all_res.bat"
    if compile_script_path.exists():
        log_message("Compiling resources...", "info")
        try:
...
def install_4k_characters_model_fix():
    mod_dir = get_versioned_path("characters")
    if not mod_dir:
        return  # Exit if mod directory is not found
    ...
    copy_files(get_versioned_path("ingame_models_x1_x2_new"), game_data_dir)
    log_message("4k Characters/Model Fix installation complete.", "success")

def install_faithful_health_bars():
    # Get the versioned mod directory
    mod_dir = Path(get_versioned_path("_faithful_hpbars"))
    if not mod_dir.exists():
        log_message(f"Mod directory '{mod_dir}' not found. Ensure the mod is downloaded and extracted correctly.", "error")
        return  # Exit if mod directory is not found

    # Use the versioned directory name in the textures path
    app_data_textures_dir = Path(GLOBAL_APPDATA_DIR) / "Load" / "Textures" / "RABAZZ" / mod_dir.name
    create_directory(app_data_textures_dir)

    # Copy files from the mod directory to the target directory
    copy_files(mod_dir, app_data_textures_dir)
...
def setup_console_ui(root):
    ...
    print_to_console("Click Check for Updates to download the current mods.", "error")
